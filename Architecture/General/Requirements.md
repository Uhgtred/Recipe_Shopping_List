...
Items are the objects that can be added to either a recipe or a shoppinglist. 

#### Price 

Each item shall have the possibility to have a price, that can be set or at a later point will be caught from a store API. With this price the overall price of a list can be calculated. For each item in the list the price will be added up, multiplied by the quantity of each item. 
...
It shall be possible to store the items in a database, so that for each item that has once been created, it is clear which unit and [price](#price) it usually has.
